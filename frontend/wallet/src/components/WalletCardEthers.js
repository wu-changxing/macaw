      ...
      window.ethereum
        ...
        .then(result => {
         const r = result[0]
          ...
          setName(r.slice(0, 6) + '...' + r.slice(-4));
            console.log(name)
          setConnButtonText(name);
          localStorage.setItem('name', name);
          localStorage.setItem('address', r);
  ...
  useEffect(() => {
    if (address && provider) {
      provider
        ...
        .catch(error => {
          setErrorMessage('Unable to fetch balance. Please try again later.');
        });
      setConnButtonText(name);
    }
  }, [address, provider]);
        ...
        <div>
            <div className='flex justify-end bg-gray-100'>
                <div className='flex flex-col justify-end divide-y divide-blue-200 bg-white text-slate-700 hover:bg-sky-500 hover:text-white border border-sky-300 focus:ring-4 focus:ring-sky-100 focus:outline-none font-medium rounded-lg text-sm px-5 py-2.5  dark:focus:ring-gray-600 dark:bg-gray-800 dark:border-gray-700 dark:text-white dark:hover:bg-gray-700 mr-2 mb-2'>
                    <button onClick={connectWalletHandler} type="button" className="inline-flex text-center items-center">
